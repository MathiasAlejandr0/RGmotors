 ...
 * como la foto de catálogo estándar de RG Motors.
 *
 * Clave = slug del vehículo · Valor = archivo en public/cars/uploads/<slug>/
 */
export const FRONT_COVER_BY_SLUG = {
...
/** URL pública de la portada 3/4 si hay mapeo. */
export function frontCoverUrl(slug) {
...
/**
 * Elige el archivo de portada entre los disponibles en disco.
 * Prioriza el mapa curado; si no hay, usa el primer archivo.
 */
export function pickFrontCoverFile(slug, files) {
  if (!files?.length) return null;
  const mapped = FRONT_COVER_BY_SLUG[slug];
  if (mapped && files.includes(mapped)) return mapped;
  // Fallback: photo-02 suele ser 3/4 frontal en lotes RG; luego photo-01 / photo-07
  const preferred =
    files.find((f) => /photo-0?2\./i.test(f)) ||
    files.find((f) => /photo-0?1\./i.test(f)) ||
    files.find((f) => /photo-0?7\./i.test(f)) ||
    files[0];
  return preferred;
}

/**
 * Asegura image + gallery[0] = portada delantera 3/4.
 * Si no hay mapeo ni galería, deja el vehículo igual.
 */
export function withFrontCover(vehicle) {
  ...
  if (gallery.length > 0 && vehicle.image !== gallery[0]) {
    return { ...vehicle, image: gallery[0] };
  }
  return vehicle;
}
